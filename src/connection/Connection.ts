...
class Connection {
  ...
  // Frames
  private readonly receivedFrames: Set<number>
  private readonly lostFrames: Set<number> 
  private lastFrame: number
  private outReliabilityIndex
  private outFrame
  private outFrameSets: Map<number, { frameSet: FrameSet, ping: bigint }>

  // Ordering
  private readonly inOrderingQueue: Map<number, Map<number, Frame>>
  private readonly inHighestIndex: number[]
  private readonly inOrderingIndex: number[]

  public constructor(raknet: Raknet, rinfo: RemoteInfo, mtuSize: number, guid: bigint) {
    this.raknet = raknet
    this.rinfo = rinfo
  ...
  private handleConnectionRequest(buffer: Buffer): void {
    ...
    // Fix this
    accepted.clientAddress = { address: this.rinfo.address, port: this.rinfo.port, version: 4 }
    accepted.requestTime = request.time
    accepted.time = BigInt(Date.now())

    return this.sendPacket(accepted)
  }
  ...
  private handleConnectedPing(buffer: Buffer): void {
    ...
    return this.sendPacket(pong)
  }

  private handleNewIncomingConnection(): void {
    this.raknet.emit(RaknetEvent.ConnectionOpened, this)
  }
  ...
  private handleGamePacket(buffer: Buffer): void {
    ...
    this.raknet.emit(RaknetEvent.GamePacket, packet, this)
  }
}
