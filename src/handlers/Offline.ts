...
class OfflineHandler {
  ...
  public handle(buffer: Buffer, rinfo: RemoteInfo): void {
    const id = buffer[0]
    switch (id) {
      default: return console.log('Unknown packet id:', id.toString(16))
      case UnconnectedPing.id: return this.handleUnconnectedPing(buffer, rinfo)
      case OpenConnectionRequest1.id: return this.handleOpenConnectionRequest1(buffer, rinfo)
      case OpenConnectionRequest2.id: return this.handleOpenConnectionRequest2(buffer, rinfo)
    }
  }
  ...
  public handleOpenConnectionRequest1(buffer: Buffer, rinfo: RemoteInfo): void {
    ...
    if (request.protocol !== 11) return console.log('Unknown protocol:', request.protocol)
    const reply = new OpenConnectionReply1()
    reply.serverGuid = this.raknet.guid
    if (buffer.byteLength + udpHeaderSize > maxMtuSize) reply.mtuSize = maxMtuSize;
  ...
  public handleOpenConnectionRequest2(buffer: Buffer, rinfo: RemoteInfo): void {
    ...
    const connection = new Connection(this.raknet, rinfo, request.mtuSize, request.clientGuid)
