...
export type Listener<T extends any[], R = unknown> = (
  ...args: T
) => Awaitable<R>;
...
export class EventEmitter<T> {
  private readonly _beforeHooks = new Map<
    keyof T,
    Listener<ForceArray<T[any]>, boolean>[]
  >();
  private readonly _afterHooks = new Map<
    keyof T,
    Listener<ForceArray<T[any]>>[]
  >();
  ...
  public async emit<K extends keyof T>(
    ...
    if ((await this.runHooks(beforeHooks, args)) === false) {
      return false;
    }

    await this.runListeners(listeners, args);

    await this.runHooks(afterHooks, args);

    return true;
  }

  private async runHooks(hooks: Function[], args: any[]): Promise<boolean> {
    const hookResults = await Promise.all(
      hooks.map(async (hook) => await hook(...args))
    );
    return !hookResults.includes(false);
  }

  private async runListeners(
    listeners: Function[],
    args: any[]
  ): Promise<void> {
    await Promise.all(
      listeners.map(async (listener) => await listener(...args))
    );
  }
